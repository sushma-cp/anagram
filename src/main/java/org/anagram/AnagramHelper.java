...
public class AnagramHelper {
    ...
    static public boolean isStringValid(String stringToValidate) {
        ...
        return stringToValidate != null && !stringToValidate.trim().isEmpty() && stringToValidate.matches("[a-zA-Z ]*");
    }

    public static void initialiseEnglishDictionary() {

        try (BufferedReader in = new BufferedReader(new FileReader("web2"));) {
            String str;
    ...
    public static void generateAllPermutationsAndKeepEnglishOnes(char[] in, StringBuffer outputString,
        ...
        for (int i = 0; i < inputLength; ++i) {
            ...
            if (alreadyUsedLetter[i])
                continue;

            outputString.append(in[i]);
            alreadyUsedLetter[i] = true;
            generateAllPermutationsAndKeepEnglishOnes(in, outputString, alreadyUsedLetter, inputLength, level + 1,
                    allWords);
